...
function installSupportLibOnCentOS5() {
    ...
    wget --no-check-certificate -O /etc/yum.repos.d/CentOS-Base.repo https://raw.githubusercontent.com/rikouu/CentOS-DXF/master/CentOS-Base.repo/CentOS-Base.repo.6.x
    yum clean all
    yum makecache
...
function downloadDXF() {
    ...
    wget -O ./dxf_server.tar.gz http://107.172.82.168:7758/dxf/dxf_server.tar.gz

    echo "下载数据库..."
    # wget -O ./mysql.tar.gz http://pxlyjtrp1.bkt.clouddn.com/mysql.tar.gz
    wget -O ./mysql.tar.gz http://107.172.82.168:7758/dxf/mysql.tar.gz
}
